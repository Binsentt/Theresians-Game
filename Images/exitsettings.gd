...
func _hide_popup() -> void:
	if not popup:
		push_warning("SettingsPopup node not found in current scene.")
		return
	...
	t.tween_property(popup, "scale", Vector2(0.85, 0.85), popup_hide_duration)
	t.parallel().tween_property(popup, "modulate:a", 0.0, popup_hide_duration)
	await t.finished
